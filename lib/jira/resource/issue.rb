...
module JIRA
  module Resource
    ...
    class Issue < JIRA::Base
      ...
      def self.jql(client, jql, options = {fields: nil, start_at: nil, max_results: nil, validate_query: true})
        url = client.options[:rest_base_path] + "/search?validateQuery=#{options[:validate_query]}"
        url << "&jql=" + CGI.escape(jql)
